...
如果希望所有设置都通过 YAML 管理，推荐使用 [三个独立脚本](WORKFLOW_SCRIPTS.md)：
生成器的 `generate_data.sh`、SideNet 的 `split_data.sh` 和 `start_training.sh`。
它们使用独立的数据输出和配置；下面的原始 Python 命令仍可直接使用。
